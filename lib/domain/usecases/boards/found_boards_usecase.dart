...
class FoundBoardsUsecase {
  final BoardRepository _boardRepository;

  FoundBoardsUsecase(this._boardRepository);

  Future<List<BoardItem>> execute({int? page, int? size}) async {
    final request = BoardsRequest(page: page ?? 0, size: size ?? 10);

    try {
      final response = await _boardRepository.getFoundBoards(request);
      
      if (response.content.isNotEmpty) {
        print('습득 게시글 목록 조회 성공 (Usecase): ${response.numberOfElements}개 항목 수신 (페이지: ${response.number})');
      } else if (response.content.isEmpty && response.empty) {
        print('습득 게시글 목록 조회 성공 (Usecase): 현재 페이지에 항목이 없습니다 (페이지: ${response.number})');
      } else {
        print('습득 게시글 목록 조회 성공 (Usecase): 예상치 못한 응답 형식입니다.');
      }
      return response.content.map((item) => item.toEntity()).toList();
    } catch (e) {
      // 네트워크 오류 또는 리포지토리에서 발생한 기타 예외 처리
      print('습득 게시글 목록 조회 유스케이스 실행 중 오류 발생: $e');
