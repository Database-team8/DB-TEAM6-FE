...
class MyBoardsUsecase {
  ...
  Future<List<BoardItem>> execute({int? page, int? size}) async {
    ...
    try {
      final response = await _boardRepository.getMyBoards(request);

      if (response.content.isNotEmpty) {
        print('내 게시글 목록 조회 성공 (Usecase): ${response.numberOfElements}개 항목 수신 (페이지: ${response.number})');
      } else if (response.content.isEmpty && response.empty) {
        print('내 게시글 목록 조회 성공 (Usecase): 현재 페이지에 항목이 없습니다 (페이지: ${response.number})');
      } else {
        print('내 게시글 목록 조회 성공 (Usecase): 예상치 못한 응답 형식입니다.');
      }
      return response.content.map((item) => item.toEntity()).toList();
    } catch (e) {
      // 네트워크 오류 또는 리포지토리에서 발생한 기타 예외 처리
      print('내 게시글 목록 조회 유스케이스 실행 중 오류 발생: $e');
      // 필요에 따라 특정 타입의 예외로 변환하여 다시 던집니다.
      // 예: throw BoardsFetchException('내 게시글을 가져오는 중 오류가 발생했습니다: ${e.toString()}');
